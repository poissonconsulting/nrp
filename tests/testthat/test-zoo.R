test_that("nrp_read_zooplankton_file works", {
  ...
  expect_error(
    nrp_read_zooplankton_file(path = path, db_path = conn, system = "columbia"),
    "'system' must be one of 'arrow', 'kootenay'."
  )

  expect_error(
    nrp_read_zooplankton_file(path = wrong_path, db_path = conn),
...
test_that("nrp_read_zooplankton works", {
  ...
  expect_error(
    nrp_read_zooplankton("not-a-path", db_path = conn),

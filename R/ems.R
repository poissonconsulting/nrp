...
nrp_extract_ems <- function(data, db_path = getOption("nrp.db_path", file.choose()), analysis_type = "standard") {
  ...
  data %<>% filter(.data$EMS_ID %in% sites$EmsSiteNumber) %>%
    mutate(
      COLLECTION_START = lubridate::ymd_hms(.data$COLLECTION_START, tz = "Etc/GMT+8"),
      COLLECTION_END = lubridate::ymd_hms(.data$COLLECTION_END, tz = "Etc/GMT+8")
    ) %>%
    select(
      .data$EMS_ID, .data$MONITORING_LOCATION, .data$COLLECTION_START, .data$COLLECTION_END, .data$REQUISITION_ID,
    ...
    mutate(
      RESULT_LETTER = gsub("M", NA, .data$RESULT_LETTER),
  ...
  if (analysis_type == "standard") {
    ...
    data %<>% filter(.data$PARAMETER %in% params_standard) %>%
      mutate(
        PARAMETER = paste0(.data$PARAMETER, "unit:", .data$UNIT),
    ...
    data %<>% map2_dfc(standard_units, fill_units) %>%
      select(
        .data$`SiteID`, .data$`COLLECTION_START`, .data$`COLLECTION_END`, .data$`REQUISITION_ID`, .data$`ANALYZING_AGENCY`,
  ...
  data %<>% mutate(
    LOWER_DEPTH = units::set_units(as.numeric(.data$LOWER_DEPTH), "m"),
...
nrp_download_ems <- function(db_path = getOption("nrp.db_path", file.choose()), start_date_time = NULL,
  ...
  result <- readwritesqlite::rws_query(query = query, conn = conn) %>%
    dplyr::mutate(
      COLLECTION_START = dttr2::dtt_date_time(.data$COLLECTION_START),
